...
begin
  ...
  if (Result = 0) then
    raise Exception.CreateFmt('load %s failed ' + UIntToStr(GetLastError), [ALibFile]);
    // use procmon to determine why DLL is not loading correctly
    // UIntToStr(GetLastError) returns code 126 - Could not find the mentioned DLL module
    // returns code 193 - Operating system architecture mismatch x32/x64
end;
